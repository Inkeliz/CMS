...
#####################################################################
# 	CASO A VARIÁVEL __DIR__ NÃO ESTEJA DISPONÍVEL (alguns servers nao funciona) 
#####################################################################
...
#####################################################################
	function _file_put_contents($file,$content){
		$bkp = date('Y_m_d_H_i_s_', time()).md5(microtime(true));
		if(file_exists($file) && is_file($file)){
			if(copy($file, $file.'__bkp_'.$bkp)){
...
#####################################################################
	function _rename($file,$newFile=null){
		if($newFile==null){ $newFile = $file; }
		$bkp = date('Y_m_d_H_i_s_', time()).md5(microtime(true));
		if(file_exists($newFile) && is_file($file)){
			if(copy($newFile, $newFile.'__bkp_'.$bkp)){
...
# 	COPIA DE ARQUIVO COM BKP
#####################################################################
	function _copy($file,$newFile=null){
		if($newFile==null){ $newFile = $file; }
		$bkp = date('Y_m_d_H_i_s_', time()).md5(microtime(true));
		if(file_exists($newFile) && is_file($file)){
			if(copy($newFile, $newFile.'__bkp_'.$bkp)){
...
##########################################################################################################
   function _erro($error) {
        echo '<pre style="position: relative;color: #F00;background-color: #FFCB00;font-weight: bold;padding: 10px;">'.PHP_EOL.
